...
Swagger — это набор инструментов, который базируется на спецификации OpenAPI 3.0 и позволяет автоматически описывать API на основе его кода. Документация, созданная через Swagger, облегчает понимание API для компьютеров и людей и описывает, как взимодействовать с приложением.

###Как устроен Swagger

На основе кода или набора правил Swagger генерирует документацию приложения в формате JSON-файла. И автоматически заботится об ее обновлении, когда вносятся изменения в код.

Swagger позволяет создавать документацию двумя способами:
1. **Автоматически на основе кода.** Инструмент «читает» код API и сам генерирует документацию. Этот способ простой, от разработчика не требуется писать что-то помимо самого кода. Но его рекомендуется применять только тогда, когда документация нужна срочно.
2. **На основе спецификации.** Этот вариант сложнее, т. к. нужно описать сущности кода, чтобы инструмент понял написанное и сгенерировал документ. Такая документация более понятна и человекочитаема. Описывать сущности необходимо в формате JSON или YAML либо в редакторе Swagger Editor.

Swagger можно разделить на четыре основных компонента:
1. **Swagger Core.** Это ядро Swagger. Позволяет автоматически генерировать документацию на основе кода при сборке проекта. Правила генерации описываются с помощью аннотаций в коде. 
2. **Swagger UI** — графический интерфейс, делающий работу со Swagger нагляднее. Он визуализирует интерактивную документацию, с которой можно взаимодействовать — отправлять запросы, перемещаться по ее содержимому и тестировать проект. 
3. **Swagger Codegen.** Этот компонент генерирует код на основе правил. Автоматически сгенерированный код решает только шаблонные задачи, так что Codegen не заменяет программиста, но избавляет его от части рутины. Позволяет создавать заглушки серверов, клиентские библиотеки API, документацию.
4. **Swagger Editor** — редактор для просмотра и изменения написанных правил. Имеет вид разделенного на две части окна: слева по спецификации пишется код описания API, справа генерируется интерфейс Swagger UI. 
...
### Установка
Перейдите на официальный сайт Docker'а и скачайте установщик Docker Desktop для своей операционной системы:

> [https://www.docker.com/products/docker-desktop/](https://www.docker.com/products/docker-desktop/)

Более подробно об установке можно почитать в официальной документации:
...
### Подготовка
Для запуска контейнера с базой данных понадобится env-файл содержащий, как минимум, пароль базы данных.
Пример этого файла уже есть в корне проекта - файл с именем `.env.example` для базы данных PostgreSQL.
Достаточно изменить имя этого файла, убрав `.example` в итоге получим файл `.env` в корне проекта.
Полученного файла будет достаточно для запуска контейнера.

Более подробно о переменных окружения для контейнера на базе официального образа PostgreSQL:
...
Для запуска контейнера откройте терминал и в терминале перейдите в корневую директорию проекта.
В корне проекта находится файл с настройками контейнеров — `docker-compose.yaml`.
(В среду разработки IntelliJ IDEA встроен терминал, который запускается в корневой директории проекта).
...
Чтобы остановить все запущенные контейнеры в текущем многоконтейнерном приложении, выполните в терминале команду
```bash
docker compose stop
```
